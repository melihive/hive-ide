...
def test_hook_setup_replaces_legacy_shell_shim(monkeypatch, tmp_path):
    home = tmp_path / "home"
    claude = home / ".claude" / "settings.json"
    claude.parent.mkdir(parents=True)
    claude.write_text(
                    ...
                    "PreCompact": [
                        {
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": "$HOME/.my_scripts/hive-ide-hook.sh --activity compacting --driver claude || true",
                                    "timeout": 10,
                                }
                            ]
                        }
    ...
    assert str(claude) in applied["changes"]
    merged = json.loads(claude.read_text(encoding="utf-8"))
    commands = [
        handler["command"]
        for groups in merged["hooks"].values()
    ...
    assert "echo keep" in commands
    assert not any("hive-ide-hook.sh" in command for command in commands)
    assert any("-m hive_ide.hook" in command for command in commands)
